...
def jobSearch(request, *args, **kwargs):
    ...
    if qs is None:
        return Response({f'There are no jobs posted at the moment'},status=404)
...
def job_create(request, *args, **kwargs):
    ...
    if serializer.is_valid(raise_exception=True):
        ## TODO: Instead of hard-coding 'tuna2' as a username, get it through request.user when you make the login feature on the client side(react)
        user = User.objects.get(username='tuna2')
        # user = request.user
